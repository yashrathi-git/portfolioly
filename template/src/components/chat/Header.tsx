...
import { Github, Link as LinkIcon, Mail, Sparkles, UserCircle2 } from "lucide-react";
        ...
        <div className="flex items-center justify-between py-3 sm:py-4">
          {showIdentity ? (
            <div className="inline-flex items-center gap-3 sm:gap-4">
              ...
              <div className="flex flex-col">
                <span className="font-semibold tracking-tight text-base sm:text-lg">{profile.name}</span>
                {profile.badge ? (
          ...
          <nav className="flex items-center gap-1.5 sm:gap-2 text-[color:var(--muted-foreground)]">
            {profile.links?.map((l) => {
              ...
              return (
                <a key={l.type} href={l.href} className="p-2 rounded-lg hover:bg-[var(--accent)] transition" aria-label={l.type}>
