  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="relative text-center text-xs text-muted-foreground">
        <span className="bg-background px-2 relative z-10">or create with email</span>
        <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 border-t" />
      </div>

      {verificationSent ? (
        <div className="space-y-3 text-sm">
          <p className="text-muted-foreground">Verification email sent to <span className="font-medium">{email}</span>. Please check your inbox and click the link to verify your account.</p>
          {error && (
